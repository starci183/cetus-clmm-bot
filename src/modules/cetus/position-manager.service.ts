...
//             return
//         }
//         await this.closePosition(PoolWithPosition)
//         await this.addLiquidityToTheNextTick(PoolWithPosition, zeroInsteadOne)
...
//         params: Partial<Record<PairId, PoolWithPosition>>,
//     ) {
//     // for example, we current work for first position and for the first pool
//         const PoolWithPosition = params[PairId.SuiIka02]
//         if (!PoolWithPosition) {
...
//             currentTick,
//         )
//         const { positions, pair } = PoolWithPosition
...
//       await this.getFirstPositionRangeStatus(PoolWithPosition)
//         if (!isOutOfRange) {
...
//                 Number(PoolWithPosition.pool.current_tick_index) -
//           Number(positions[0].tick_upper_index),
//             )
//             this.logger.verbose(
//                 `Tick diff: ${tickDiff}, tick spacing: ${PoolWithPosition.pool.tickSpacing}.`,
//             )
//             if (
//                 !this.tickManagerService.checkEligibleToClosePosition(
//                     tickDiff,
//                     Number(PoolWithPosition.pool.tickSpacing),
//                 )
//             ) {
//                 this.logger.verbose(
...
//   async closePosition({ pool, positions }: PoolWithPosition) {
//       // Fetch position data
//       const position = positions[0]
//       if (!position) {
//           throw new Error("Position not found")
